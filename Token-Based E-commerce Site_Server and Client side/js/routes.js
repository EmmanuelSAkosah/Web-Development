...
let loggedInUser = {user : null, tokens : null};
...
app.post('/login', urlencodedParser, async (req, res) => {
  if (model.authenticate(req.body.uname, req.body.pword) === true) {
    ...
    loggedInUser.user = req.body.uname;  
      
      //get tokens of user      
    loggedInUser.tokens = await model.getAUsersToken(req.body.uname); 
       
    //console.log("model funct returned "+ usertokens);
    res.redirect('/'); 
  }
  else {
    req.session.destroy(()=>{});  // Safe asyncrohous call
...
app.get('/logout', (req, res) => {
  //reset loggedInUser
  req.session.destroy(()=>{});   // Safe asyncrohonus call
    //clear the loggedInUser var
  loggedInUser.user = null; 
  loggedInUser.tokens = null;
  res.redirect('/login');
});


app.get('/', (req, res) => {   
    res.render('index.ejs', { title: 'Main', loggedInUser: loggedInUser});    
});
...
app.get('/listItems', (req, res) => {
  ...
  res.render('listItems.ejs', { title: 'Item Listing',loggedInUser: loggedInUser });
});
...
// view list of redeemed items  on server side
app.get('/listRedeemedItems', async (req, res) => {
  try {  
   
    let redeemedItemsList = await model.getRedeemedItemsList();

    // Render the view
    res.render('listredeemedItems.ejs', { title: 'Redeemed Items Listing', redeemedItemsList: redeemedItemsList });
  } catch (err) {
    console.error(err);
    res.status(500).send('Error!');
  }
...
// server side, view redeemed items.
app.get('/redeemeditems', async (req, res) => {

  try {
   
    let data = await model.getRedeemedItems();
    // Data can be null if not found
    res.render('item.ejs', { title: 'Item', data: data, Query: Query });
  } catch (err) {
    console.error(err);
